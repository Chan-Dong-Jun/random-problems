...
let sum = function(arrayOfNums) {
    let res = 0;
    console.log(typeof arrayOfNums[1])
    for (let index=0; index<arrayOfNums.length; index++) {
        res += arrayOfNums[index];
    }
    return res;
}
...
let reverseArrayInPlace = function(inputArray) {
    ...
    for (let i=0; i<Math.floor(inputArray.length/2); i++) {
        ...
        inputArray[inputArray.length-i-1] = temp1;
...
For example, if I create two new values {value: 0, rest: list} and {value: -1, rest: list} (with list referring to the binding defined earlier), they are both independent lists, but they share the structure that makes up their last three elements. The original list is also still a valid three-element list.
...
let arrayToList = function(array) {
    ...
    for (let i=0; i<array.length; i++) {
        let newNode = {value:array[i], rest:null}
        if (res===null) {
            res = newNode;
            curr = res;
        } else {
...
let prepend = function(value, list=null) {
    let newNode = {value, rest:list};
    return newNode;
}
...
let deepEqual = function(obj1, obj2) {
    ...
    if (obj1Keys == null || obj2Keys == null) {
        return false;
    }
